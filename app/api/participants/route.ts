...
export async function POST(request: NextRequest) {
  try {
    const body = await request.json() as { deviceId?: string; eventDate?: string; consent?: boolean; fullName?: string; gender?: Gender; ageRange?: AgeRange };
    ...
    if (body.consent && (!body.fullName?.trim() || !['18-24','25-45','45+'].includes(body.ageRange || ''))) return NextResponse.json({ error: 'Vui lòng điền đầy đủ thông tin.' }, { status: 400 });
    ...
    const participant = await db.runTransaction(async (transaction) => {
      ...
      const data = {
        ...
        gender: body.gender, ...(body.consent ? { fullName: body.fullName!.trim(), ageRange: body.ageRange } : {}),
